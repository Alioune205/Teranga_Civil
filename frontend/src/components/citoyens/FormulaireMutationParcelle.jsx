// =============================================================================
// FormulaireMutationParcelle.jsx
// Formulaire de création de demande de mutation de parcelle pour le Guichet Rapide.
// =============================================================================
...
// ─── Composant Principal ─────────────────────────────────────────────────────
export default function FormulaireMutationParcelle({ citoyenId, citoyen: citoyenProp, paymentData, onSuccess, onCancel }) {
  ...
  const validerFormulaire = () => {
    ...
    if (!formData.superficie.trim() || parseFloat(formData.superficie) <= 0) nouvellesErreurs.superficie = 'La superficie doit être renseignée et supérieure à 0';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const montantCalcule = (parseFloat(formData.superficie) || 0) * 1000;
      payload.append('type_document', 'mutation_parcelle');
      payload.append('motif', 'Guichet Rapide');
      if (paymentData) {
        payload.append('paiement_mode', paymentData.mode || 'Espèces');
        payload.append('montant', montantCalcule);
      }
      ...
      let errorMessage = "Erreur lors de l'enregistrement de la demande de mutation de parcelle";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-xl">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-blue-800 dark:text-blue-300 font-jakarta">
            Demande de Mutation de Parcelle
          </h3>
          {loadingCitoyen ? (
      ...
      <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800 p-4 rounded-lg flex justify-between items-center">
        <div>
          <h4 className="font-semibold text-emerald-800 dark:text-emerald-300">Frais de mutation</h4>
          <p className="text-sm text-emerald-600 dark:text-emerald-400">1 000 FCFA par mètre carré</p>
        </div>
        <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
          {((parseFloat(formData.superficie) || 0) * 1000).toLocaleString('fr-FR')} FCFA
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <FileUploadField name="acte_vente" label="Acte de vente" description="Charger l'acte" />
        <FileUploadField name="acte_original_terrain" label="Acte original du terrain" description="Charger l'acte" />
        <FileUploadField name="plan_cadastral" label="Plan cadastral" description="Charger le plan" />
        <FileUploadField name="cni_vendeur" label="CNI du vendeur" description="Charger la CNI" />
        <FileUploadField name="cni_acheteur" label="CNI de l'acheteur" description="Charger la CNI" />
        <FileUploadField name="cni_temoin1" label="CNI du premier témoin" description="Charger la CNI" />
        <FileUploadField name="cni_temoin2" label="CNI du second témoin" description="Charger la CNI" />
      </div>
