...
def _draw_mutation_receipt_content(p, width, height, dossier, cachet_path, signature_path, cachet_nominal_path, qr_image_reader):
    ...
    p.setFont("Helvetica", 10)
    p.drawString(6*cm, y, "Mutation de parcelle")
    y -= 30
    ...
    docs = [
        "Acte de vente", "Acte original du terrain", "Plan cadastral",
        "Copie CNI du vendeur", "Copie CNI de l'acheteur",
        "Copie CNI des deux témoins"
    ]
...
def generate_mutation_receipt(dossier):
    ...
    _draw_mutation_receipt_content(
